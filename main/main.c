                              ...
                              int32_t event_id, void *event_data)
{
    ...
    case ETHERNET_EVENT_CONNECTED:
        ...
        ESP_LOGI(MAIN_TAG, "Ethernet Link Up");
        ESP_LOGI(MAIN_TAG, "Ethernet HW Addr %02x:%02x:%02x:%02x:%02x:%02x",
                 mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
        break;
    case ETHERNET_EVENT_DISCONNECTED:
        ESP_LOGI(MAIN_TAG, "Ethernet Link Down");
        break;
    case ETHERNET_EVENT_START:
        ESP_LOGI(MAIN_TAG, "Ethernet Started");
        break;
    case ETHERNET_EVENT_STOP:
        ESP_LOGI(MAIN_TAG, "Ethernet Stopped");
        break;
    default:
        break;
    }
                                 ...
                                 int32_t event_id, void *event_data)
{
    ...
    ESP_LOGI(MAIN_TAG, "Ethernet Got IP Address");
    ESP_LOGI(MAIN_TAG, "~~~~~~~~~~~");
    ESP_LOGI(MAIN_TAG, "ETHIP:" IPSTR, IP2STR(&ip_info->ip));
    ESP_LOGI(MAIN_TAG, "ETHMASK:" IPSTR, IP2STR(&ip_info->netmask));
    ESP_LOGI(MAIN_TAG, "ETHGW:" IPSTR, IP2STR(&ip_info->gw));
    ESP_LOGI(MAIN_TAG, "~~~~~~~~~~~");
}
